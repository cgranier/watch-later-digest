...
- Grok Bot's routine-create path (2026-09-16) accepts name, schedule, prompt and enabled only: no maximum-runtime or inference-budget field, so rule 5's caps cannot be set on the routine. Check the routine card in the app for caps and set them there; until the platform exposes them, the video cap and the transcript-size skip live in the profile and the skill text only, and the digest should say so once.
...
Durability on Grok Bot, measured 2026-09-16 (T12): a marker in STATE_DIR under `/workspace` and one in `$HOME` both survived "Update Grok Bot's Computer", and a pip-installed `yt-dlp` in `~/.local/bin` was still on PATH afterwards, so bootstrap was a no-op. The runbook's own account is more conservative: an update keeps files and logins and reinstalls the base image, so stock `python3`, `git` and `curl` are present afterwards; anything pip-installed, including `yt-dlp`, is gone and `bootstrap.sh` puts it back. Step 0 and bootstrap cover that as written. Design to the runbook, not to the one measurement: keep state under `/workspace`, keep the bootstrap line first, and treat a surviving `$HOME` as luck.
